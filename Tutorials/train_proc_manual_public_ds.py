...
from PluginCore.Decoder.trainer.utils import (
    plot_res,
    boxplot_res,
    param3d_viz,
    return_df_search,
...
from PluginCore.Processor.module.temp import (
    exponential_moving_standardize,
    convert_to_uV,
)
from PluginCore.Decoder.trainer.Skorch import (
    SkorchFactory,
    SkorchRandSearch,
    SkorchGridSearch,
)
from PluginCore.Decoder.trainer.csp import (
    CSPFactory,
    CSPRandomSearch,
    CSPFilter,
    CSPWrapper,
...
# if file_dir isnot exist creat one
...
alg = {
    ...
    7: CSPWrapper(
        train_factory=CSPFactory(Model=FBCSP),
        inspector_factory=CSPInspector(InspectorSyn(pos_label=None)),
    ),
}
...
eegnet = EEGNetv4(
    in_chans=22, n_classes=4, input_window_samples=200, final_conv_length="auto"
)


(
    re_subjects_fbcsp,
    trainned_model_subjects_fbcsp,
    df_subjects_fbcsp,
) = core.run_cv_subject_dependent(
    preprocesser_id=2,
    algorithm_id=2,
    inspector_id=2,
    n_fold=5,
    ...
    model_name="FBCSP",
    metrics=["acc", "kappa", "model", "subject"],
)
(
    re_subjects_shallow,
    trainned_model_subjects_shallow,
    df_subjects_shallow,
) = core.run_cv_subject_dependent(
    preprocesser_id=1,
    algorithm_id=1,
...
(
    re_subjects_eegnet,
    trainned_model_subjects_eegnet,
    df_subjects_eegnet,
) = core.run_cv_subject_dependent(
    preprocesser_id=1,
    algorithm_id=1,
...
if train_csp:
    ...
    core.log_model_search(
        gs=gs_fbcsp, keys=["select_ratio", "h_freq", "n_cuts"], dir=file_dir
    )
else:
    df_search = return_df_search(
        gs_cnn, ["module__n_filters_time", "module__n_filters_spat", "module_drop_prob"]
    )
    ...
    param3d_viz(
        gs=gs_cnn,
        params=["module__n_filters_time", "module__n_filters_spat", "module_drop_prob"],
    )
    core.log_model_search(
        gs=gs_cnn,
        keys=["module__n_filters_time", "module__n_filters_spat", "module_drop_prob"],
        dir=file_dir,
    )
...
(
    re_subjects_fbcsp_train,
    trainned_model_subjects_fbcsp_train,
    df_subjects_fbcsp_train,
) = core.run_cv_subject_dependent(
    preprocesser_id=2,
    algorithm_id=2,
    inspector_id=2,
...
core.track_time(
    ...
    trial_start_offset_seconds=0,
    trial_end_offset_seconds=0,
    model=core.modules["trained_module"],
    n_inter=4,
...
i_train_fold = 0
(train_Xs, train_ys), (test_Xs, test_ys) = core.provide_Xys(
    dataset_id=1,
    preprocess_id=2,
    subject_mode="subject_dependent",
...
core.algorithms[6].visualize_train_test_dist(
    train_X=train_Xs[i_train_fold],
    train_y=train_ys[i_train_fold],
    test_X=test_Xs[i_train_fold],
    test_y=test_ys[i_train_fold],
)
...
temporal_res = core.feature_analysis_wrapper(
    preprocess_id=2,
    algorithm_id=7,
    dataset_id=1,
    subject_mode="subject_dependent",
...
spat_res = core.feature_analysis_wrapper(
    preprocess_id=2,
    algorithm_id=7,
    dataset_id=1,
    subject_mode="subject_dependent",
...
info = mne.create_info(
    ch_names=raw.info["ch_names"], sfreq=1, ch_types=["eeg"] * len(raw.info["ch_names"])
)
