...
class MorletConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        t_tensor = torch.from_numpy(
            np.linspace(0, window_len_time, window_len_point).astype(np.float32) - (window_len_time / 2))
        if self.device == 'cuda':
            t_tensor = t_tensor.cuda()
        kernel = torch.exp(
            -.5 * (torch.pow(self.a.mm(t_tensor.reshape(1, self.kernel_size[0] * self.kernel_size[1])), 2))) * (
                     torch.cos(
                         self.b.mm(2.0 * 3.1415926 * t_tensor.reshape(1, self.kernel_size[0] * self.kernel_size[1]))))
...
class WaSFCNNMorlet(nn.Module):
    def __init__(self, n_chan, time_step, n_classes, n_filter_time, n_filter_spat, fs, device='cuda'):
        ...
        self.time_conv = MorletConv2d(channel_in=1, channel_out=self.n_filter_time, kernel_size=(1, 25),
                                      sample_rate=self.fs)
