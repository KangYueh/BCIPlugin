...
from .core.modules import Expression, Ensure4d ,squeeze_final_output
...
class EEGNetv4(nn.Sequential):
    ...
    def predict(self, x):
        i_pres = self.forward(x)
        if self.classes is None:
            return i_pres
        else:
            pre = torch.ones_like(i_pres)
            for i,_ in enumerate(i_pres):
                pre[i] = self.classes[i_pres[i]]
            return pre
...
class EEGNetv1(nn.Sequential):
    ...
    def __init__(
        ...
        self.add_module("elu_1", Expression(elu))
        # transpose to Tutorials x 1 x (virtual, not EEG) channels x time
        self.add_module(
