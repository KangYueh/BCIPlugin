...
def get_array_from_container(container):
    """ concatenate them into a single numpy array (2d) and return it

    Parameters
    ----------
    container: list
        a list of dict. The structure of this dict is {'eeg': data, 'receivedSamples': sample.shape[1]}

    Returns
    -------
        eeg_array: numpy.ndarray
            2-dim eeg data
    """
    data = [c['eeg'] for c in container]
    eeg_array = np.concatenate(data, axis=1)
    return eeg_array


class LSLClient(StreamWindow, StreamRecorder):
    def __init__(self, channel_names, buffer_period=5, verbose=False, lsl_type='EEG'):
        self.BufferPeriod = buffer_period
        self.verbose = verbose
        self.inlet = None
        self.LSLType = lsl_type
    ...
    def stopRecording(self, **kwargs):
        ...
        data = get_array_from_container(self.raw_container)
        # convert to uv
...
lc = LSLClient(channel_names=ch_names)
lc.startStreaming()
